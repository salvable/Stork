...
it('POST /stork/purchase/:userId/:accountId 200', async () => {
    const response = await request(app).post(`/stork/purchase/${userId}/${accountId}?storkName=${encodeURI("삼성전자")}&number=14&price=70000`).set(
...
it('POST /stork/purchase/:userId/:accountId 400', async () => {
    const response = await request(app).post(`/stork/purchase/${userId}/${accountId}?storkName=${encodeURI("삼성전자")}`).set(
        'Authorization' ,  `Bearer ${accessToken}`
    );
...
it('POST /stork/purchase/:userId/:accountId 2 404', async () => {
    const response = await request(app).post(`/stork/purchase/${userId}/NotFoundTest?storkName=${encodeURI("삼성전자")}&number=5&price=70000`).set(
        'Authorization' ,  `Bearer ${accessToken}`
    );
